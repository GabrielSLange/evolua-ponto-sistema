...
const EditEstabelecimentoScreen = () => {
   ...
   const [initialData, setInitialData] = useState<EstabelecimentoFormData | undefined>(undefined);
   ...
   useEffect(() => {
      if (id) {
         setLoading(true);
         api.get<{ data: Estabelecimento }>(`/estabelecimento?estabelecimentoId=${id}`)
   ...
   const handleUpdate = async (data: EstabelecimentoFormData) => {
      setLoading(true);
      try {
   ...
   return (
      <ScreenContainer>
         <View style={{ flex: 1 }}>
            <Appbar.Header>
               <Appbar.BackAction onPress={() => router.push(`/(superadmin)/estabelecimentos/estabelecimentos`)} />
